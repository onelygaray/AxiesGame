...
@section('content')
    ...
    @if ($errors->has('message'))
        <div class="w-[100px] h-[20px] text-lg text-white border-white bg-transparent">
    ...
    <section class="mt-10 mx-auto flex w-[800px] justify-between h-[800px] ">
        ...
        <div class="mx-auto w-[400px] h-][700px] flex  gap-10">
            ...
            <form id="form" action="{{ route('collection.store') }}" method="POST" enctype="multipart/form-data">
                ...
                @enderror
                <input name="name"
                    class="text-pink-50 h-[48px] w-full bg-transparent rounded-[8px] border border-[#343444]" type="text"
                    placeholder="Insert Name collection">
                @error('name')
                    {{ $message }}
                @enderror
